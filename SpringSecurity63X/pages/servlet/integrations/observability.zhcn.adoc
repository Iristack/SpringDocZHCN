...
Spring Security 开箱即用地与 Spring Observability 集成，支持追踪（tracing）；同时也可以轻松配置以收集指标（metrics）。
...
对于非 Spring Boot 应用程序，或需要覆盖现有 Boot 配置的情况，你可以自行发布 `ObservationRegistry` Bean，Spring Security 仍会自动使用它。
...
如果你不希望启用任何 Spring Security 的观测功能，在 Spring Boot 应用中可以注册一个 `ObservationRegistry.NOOP` 的 `@Bean`。  
但请注意，这可能会关闭除 Spring Security 外的其他组件的观测功能。

更推荐的方式是通过 `ObservationPredicate` 来修改现有的 `ObservationRegistry`，例如：
...
[TIP]
XML 配置方式目前不支持禁用观测功能。  
若要避免启用，只需不要设置 `observation-registry-ref` 属性即可。
...
1. `spring.security.http.requests` — 包裹整个过滤器链（包括请求）的跨度
...
5. `spring.security.http.unsecured.requests` — 包裹未被 Spring Security 保护的请求的跨度
...
`spring.security.http.chains.before` + `spring.security.http.chains.after` = 请求中由 Spring Security 负责的部分